          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            {assistants.map((coach, i) => (
              <FadeIn key={coach.name} delay={i * 60}>
                <div className="border border-gray-200 p-6 md:p-8 hover:border-orange/40 hover:bg-cream transition-all duration-300 text-center">
                  <div className="w-20 h-20 mx-auto mb-5 bg-gray-100 rounded-full flex items-center justify-center">
                    <span className="text-xl font-extrabold font-heading text-charcoal">
                      {coach.name.split(" ").map(n => n[0]).join("")}
                    </span>
                  </div>
                  <h3 className="text-xl md:text-2xl font-extrabold font-heading uppercase text-charcoal">
                    {coach.name}
                  </h3>
                  <p className="text-sm font-semibold tracking-[0.1em] uppercase text-gray-500 font-body mt-2">
                    {coach.role}
                  </p>
                </div>
              </FadeIn>
